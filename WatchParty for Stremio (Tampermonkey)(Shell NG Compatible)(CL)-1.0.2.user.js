// ==UserScript==
// @name         WatchParty for Stremio (Tampermonkey)(Shell NG Compatible)(CL)
// @namespace    http://tampermonkey.net/
// @version      1.0.2
// @description  Progress-Based Sync + Player API + Better Auto-Load (Works with Shell NG)
// @author       Hamza
...
(function() {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initWatchParty);
    } else {
        initWatchParty();
    }

    function initWatchParty() {
        console.log("[WatchParty] v22.2 Userscript Loaded for Shell NG!");
        ...
        /** ✅ Enhanced Player API for Shell NG */
        const CustomPlayerAPI = {
            get video() {
                // Try multiple selectors for different Stremio versions/players
                return document.querySelector("video") ||
                       document.querySelector("[data-testid='video-player'] video") ||
                       document.querySelector(".player-wrapper video") ||
            ...
            set paused(value) {
                if (this.video) {
                    if (value) {
                        ...
                        this.video.play().catch(() => console.warn("Autoplay blocked"));
                    }
                }
            },
            get playbackSpeed() {
        ...
        window.WatchPartyAPI = {
                ...
                const observer = new MutationObserver((mutations) => {
                    mutations.forEach((mutation) => {
                        if (mutation.type === 'childList') {
                            ...
                            if (video && !video.hasWatchPartyListeners) {
                                this.observeVideo();
                            }
                        }
                    });
            ...
            get video() {
                return CustomPlayerAPI.video;
            },

            observeVideo() {
                const video = this.video;
                if (!video || video.hasWatchPartyListeners) return;

                console.log("[WatchParty] Attaching video listeners");
                video.hasWatchPartyListeners = true;
        ...
        setTimeout(() => WatchPartyAPI.init(), 1000);
        ...
        /** ✅ Enhanced UI Button Injection for Shell NG */
        function injectButton() {
            if (document.getElementById(BUTTON_ID)) return;

            // Try multiple selectors for different Stremio layouts
            const selectors = [
                'nav[class*="horizontal-nav-bar"] div[class*="buttons-container"]',
                '.navbar .buttons-container',
                '.top-bar .controls',
                '[class*="nav"] [class*="button"]',
                '.player-controls'
            ];
            ...
            if (!buttonContainer) {
                const nav = document.querySelector('nav') || document.querySelector('.navbar') || document.querySelector('.top-bar');
                if (nav) {
        ...
        // More aggressive button injection for Shell NG
        const buttonInjectionInterval = setInterval(injectButton, 1500);
        setTimeout(() => clearInterval(buttonInjectionInterval), 30000); // Stop after 30 seconds

        /** ✅ Modal UI (same as original) */
        function openModal() {
            ...
            if (!modal) {
                ...
                modal.style.cssText = `
                    ...
                    width:360px;z-index:99999;text-align:center;box-shadow:0 0 15px rgba(0,0,0,0.5);
                `;
                modal.innerHTML = `
                    <h3>🎉 Watch Party (Shell NG)</h3>
